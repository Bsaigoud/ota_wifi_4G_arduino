...
public:
    ...
    virtual std::unique_ptr<TinyGsmClient> create()
    {
        return std::unique_ptr<TinyGsmClient>(new TinyGsmClient());
    }

    virtual bool verify(TinyGsmClient &client, const char *host)
    {
        return true;
    }
};
...
public:
    ...
    std::unique_ptr<TinyGsmClient> create() override
    {
        return std::unique_ptr<TinyGsmClient>(new TinyGsmClientSecure());
    }

    bool verify(TinyGsmClient &client, const char *host) override
    {
        TinyGsmClientSecure &wcs = static_cast<TinyGsmClientSecure &>(client);
        if (_cacert == nullptr)
        {
            wcs.setInsecure();
        }
 ...
 * @return success bool
 */
bool HTTPClient_gsm::begin(TinyGsmClient &client, String url)
{
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if (_tcpDeprecated)
    {
 ...
 * @return success bool
 */
bool HTTPClient_gsm::begin(TinyGsmClient &client, String host, uint16_t port, String uri, bool https)
{
#ifdef HTTPCLIENT_1_1_COMPATIBLE
    if (_tcpDeprecated)
    {
...
#ifdef HTTPCLIENT_1_1_COMPATIBLE
bool HTTPClient_gsm::begin(String url, const char *CAcert)
{
 ...
 * @param url String
 */
bool HTTPClient_gsm::begin(String url)
{
    if (_client && !_tcpDeprecated)
    {
        log_d("mix up of new and deprecated api");
...
bool HTTPClient_gsm::beginInternal(String url, const char *expectedProtocol)
{
...
#ifdef HTTPCLIENT_1_1_COMPATIBLE
bool HTTPClient_gsm::begin(String host, uint16_t port, String uri)
{
    if (_client && !_tcpDeprecated)
    {
        log_d("mix up of new and deprecated api");
...
bool HTTPClient_gsm::begin(String host, uint16_t port, String uri, const char *CAcert)
{
    if (_client && !_tcpDeprecated)
    {
        log_d("mix up of new and deprecated api");
...
bool HTTPClient_gsm::begin(String host, uint16_t port, String uri, const char *CAcert, const char *cli_cert, const char *cli_key)
{
    if (_client && !_tcpDeprecated)
    {
        log_d("mix up of new and deprecated api");
 ...
 */
void HTTPClient_gsm::disconnect(bool preserveClient)
{
    if (connected())
    {
 ...
 */
bool HTTPClient_gsm::connected()
{
    if (_client)
    {
#if DEBUG_ENABLE
        Serial.println("connected: " + String(_client->connected()));
#endif
 ...
 * @param reuse bool
 */
void HTTPClient_gsm::setReuse(bool reuse)
{
    _reuse = reuse;
}
 ...
 */
void HTTPClient_gsm::setUserAgent(const String &userAgent)
{
 ...
 */
void HTTPClient_gsm::setAuthorization(const char *user, const char *password)
{
    if (user && password)
    {
        String auth = user;
 ...
 */
void HTTPClient_gsm::setAuthorization(const char *auth)
{
    if (auth)
    {
 ...
 */
void HTTPClient_gsm::setAuthorizationType(const char *authType)
{
    if (authType)
    {
 ...
 */
void HTTPClient_gsm::setConnectTimeout(int32_t connectTimeout)
{
 ...
 */
void HTTPClient_gsm::setTimeout(uint16_t timeout)
{
    _tcpTimeout = timeout;
    if (connected())
    {
#if DEBUG_ENABLE
        Serial.println("tcpTimeout");
#endif
 ...
 * @param use
 */
void HTTPClient_gsm::useHTTP10(bool useHTTP10)
{
 ...
 * @return http code
 */
int HTTPClient_gsm::POST(uint8_t *payload, size_t size)
{
    return sendRequest("POST", payload, size);
}

int HTTPClient_gsm::POST(String payload)
{
 ...
 * @return http code
 */
int HTTPClient_gsm::PATCH(uint8_t *payload, size_t size)
{
    return sendRequest("PATCH", payload, size);
}

int HTTPClient_gsm::PATCH(String payload)
{
 ...
 * @return http code
 */
int HTTPClient_gsm::PUT(uint8_t *payload, size_t size)
{
    return sendRequest("PUT", payload, size);
}

int HTTPClient_gsm::PUT(String payload)
{
 ...
 * @return
 */
int HTTPClient_gsm::sendRequest(const char *type, String payload)
{
 ...
 */
int HTTPClient_gsm::sendRequest(const char *type, uint8_t *payload, size_t size)
{
    int code;
    bool redirect = false;
    ...
    do
    {
        ...
        if (!connect())
        {
            return returnError(HTTPC_ERROR_CONNECTION_REFUSED_GSM);
        }
        ...
        // {
        //     addHeader("Cookie_eth", cookie_string);
        // }

        // send Header
        if (!sendHeader(type))
        {
            return returnError(HTTPC_ERROR_SEND_HEADER_FAILED_GSM);
        }
        ...
        if (payload && size > 0)
        {
            ...
            if (sent_bytes != size)
            {
                return returnError(HTTPC_ERROR_SEND_PAYLOAD_FAILED_GSM);
            }
        }
        ...
        redirect = false;
        if (
            _followRedirects != HTTPC_DISABLE_FOLLOW_REDIRECTS_GSM &&
            ...
            _location.length() > 0)
        {
            ...
            case HTTP_CODE_MOVED_PERMANENTLY_GSM:
            case HTTP_CODE_TEMPORARY_REDIRECT_GSM:
            {
                if (
                    // allow to force redirections on other methods
                    // (the RFC require user to accept the redirection)
                    _followRedirects == HTTPC_FORCE_FOLLOW_REDIRECTS_GSM ||
...
#if DEBUG_ENABLE
                        Serial.println("failed setting URL for redirection");
#endif
                        // no redirection
                        break;
                    }
            ...
            case HTTP_CODE_FOUND_GSM:
            case HTTP_CODE_SEE_OTHER_GSM:
            {
                redirectCount += 1;
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClient_gsm::sendRequest(const char *type, Stream *stream, size_t size)
{

    if (!stream)
    {
        return returnError(HTTPC_ERROR_NO_STREAM_GSM);
    }
    ...
    int buff_size = HTTP_TCP_BUFFER_SIZE_GSM;
    ...
    if ((len > 0) && (len < HTTP_TCP_BUFFER_SIZE_GSM))
    {
        buff_size = len;
    }
    ...
    {
        log_d("too less ram! need %d", HTTP_TCP_BUFFER_SIZE_GSM);
        return returnError(HTTPC_ERROR_TOO_LESS_RAM_GSM);
    }
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClient_gsm::getSize(void)
{
    return _size;
}
 ...
 */
TinyGsmClient &HTTPClient_gsm::getStream(void)
{
    if (connected())
    {
        return *_client;
 ...
 * @return TinyGsmClient*
 */
TinyGsmClient *HTTPClient_gsm::getStreamPtr(void)
{
    if (connected())
    {
        return _client;
 ...
 */
int HTTPClient_gsm::writeToStream(Stream *stream)
{
    ...
    if (!connected())
    {
        return returnError(HTTPC_ERROR_NOT_CONNECTED_GSM);
    }
    ...
    if (_transferEncoding == HTTPC_TE_IDENTITY_GSM)
    {
    ...
    else if (_transferEncoding == HTTPC_TE_CHUNKED_GSM)
    {
        int size = 0;
        while (1)
        {
            if (!connected())
            {
                return returnError(HTTPC_ERROR_CONNECTION_LOST_GSM);
            }
            String chunkHeader = _client->readStringUntil('\n');

            if (chunkHeader.length() <= 0)
            {
                return returnError(HTTPC_ERROR_READ_TIMEOUT_GSM);
            }
            ...
            else
            {
                ...
                if (ret != _size)
                {
                    return returnError(HTTPC_ERROR_STREAM_WRITE_GSM);
                }
                break;
            }
    ...
    {
        return returnError(HTTPC_ERROR_ENCODING_GSM);
    }
 ...
 * @return String
 */
String HTTPClient_gsm::getString(void)
{
 ...
 * @return String
 */
String HTTPClient_gsm::errorToString(int error)
{
    switch (error)
    {
    case HTTPC_ERROR_CONNECTION_REFUSED_GSM:
        return F("connection refused");
    case HTTPC_ERROR_SEND_HEADER_FAILED_GSM:
        return F("send header failed");
    case HTTPC_ERROR_SEND_PAYLOAD_FAILED_GSM:
        return F("send payload failed");
    case HTTPC_ERROR_NOT_CONNECTED_GSM:
        return F("not connected");
    case HTTPC_ERROR_CONNECTION_LOST_GSM:
        return F("connection lost");
    case HTTPC_ERROR_NO_STREAM_GSM:
        return F("no stream");
    case HTTPC_ERROR_NO_HTTP_SERVER_GSM:
        return F("no HTTP server");
    case HTTPC_ERROR_TOO_LESS_RAM_GSM:
        return F("too less ram");
    case HTTPC_ERROR_ENCODING_GSM:
        return F("Transfer-Encoding not supported");
    case HTTPC_ERROR_STREAM_WRITE_GSM:
        return F("Stream write error");
    case HTTPC_ERROR_READ_TIMEOUT_GSM:
 ...
 * @param first
 */
void HTTPClient_gsm::addHeader(const String &name, const String &value, bool first, bool replace)
{
...
void HTTPClient_gsm::collectHeaders(const char *headerKeys[], const size_t headerKeysCount)
{
...
String HTTPClient_gsm::header(const char *name)
{
    for (size_t i = 0; i < _headerKeysCount; ++i)
    {
...
String HTTPClient_gsm::header(size_t i)
{
...
String HTTPClient_gsm::headerName(size_t i)
{
    if (i < _headerKeysCount)
    {
...
int HTTPClient_gsm::headers()
{
    return _headerKeysCount;
}

bool HTTPClient_gsm::hasHeader(const char *name)
{
    for (size_t i = 0; i < _headerKeysCount; ++i)
    {
 ...
 */
bool HTTPClient_gsm::connect(void)
{
    uint8_t count = 0;
    if (connected())
    {
    ...
    if (!_client)
    {
        log_d("HTTPClient_gsm::begin was not called or returned error");
#if DEBUG_ENABLE
        Serial.println("HTTPClient_gsm::begin was not called or returned error");
#endif
        return false;
    }
#ifdef HTTPCLIENT_1_1_COMPATIBLE
...
#if DEBUG_ENABLE
        Serial.println("failed connect to " + _host + ":" + String(_port));
#endif
        return false;
    }

    // set Timeout for TinyGsmClient and for Stream::readBytesUntil() and Stream::readStringUntil()
 ...
 * @return status
 */
bool HTTPClient_gsm::sendHeader(const char *type)
{
    if (!connected())
    {
        return false;
 ...
 */
int HTTPClient_gsm::handleHeaderResponse()
{

    if (!connected())
    {
        return HTTPC_ERROR_NOT_CONNECTED_GSM;
    }
    ...
    _transferEncoding = HTTPC_TE_IDENTITY_GSM;
    ...
    while (connected())
    {
        ...
        if (len > 0)
        {
            ...
            if (headerLine == "")
            {
                ...
                if (transferEncoding.length() > 0)
                {
                    ...
                    {
                        _transferEncoding = HTTPC_TE_CHUNKED_GSM;
                    }
                    ...
                    {
                        return HTTPC_ERROR_ENCODING_GSM;
                    }
                }
                else
                {
                ...
                else
                {
                    ...
                    return HTTPC_ERROR_NO_HTTP_SERVER_GSM;
                }
            }
        }
        else
        {
            ...
            {
                return HTTPC_ERROR_READ_TIMEOUT_GSM;
            }
            delay(10);
        }
    }

    return HTTPC_ERROR_CONNECTION_LOST_GSM;
}
 ...
 */
int HTTPClient_gsm::writeToStreamDataBlock(Stream *stream, int size)
{
    int buff_size = HTTP_TCP_BUFFER_SIZE_GSM;
    int len = size;
    int bytesWritten = 0;
            ...
            if (sizeAvailable)
            {
                ...
                if (bytesWrite != bytesRead)
                {
                    ...
                    if (bytesWrite != leftBytes)
                    {
                        ...
                        free(buff);
                        return HTTPC_ERROR_STREAM_WRITE_GSM;
                    }
                }
    ...
    {
        log_w("too less ram! need %d", HTTP_TCP_BUFFER_SIZE_GSM);
        return HTTPC_ERROR_TOO_LESS_RAM_GSM;
    }
 ...
 * @return error
 */
int HTTPClient_gsm::returnError(int error)
{
    if (error < 0)
    {
...
void HTTPClient_gsm::setFollowRedirects(followRedirects_t_gsm follow)
{
    _followRedirects = follow;
}

void HTTPClient_gsm::setRedirectLimit(uint16_t limit)
{
 ...
 * @param url
 */
bool HTTPClient_gsm::setURL(const String &url)
{
...
const String &HTTPClient_gsm::getLocation(void)
{
    return _location;
}

void HTTPClient_gsm::setCookieJar(CookieJar_gsm *cookieJar)
{
    _cookieJar = cookieJar;
}

void HTTPClient_gsm::resetCookieJar()
{
    _cookieJar = nullptr;
}

void HTTPClient_gsm::clearAllCookies()
{
    if (_cookieJar)
        _cookieJar->clear();
}

void HTTPClient_gsm::setCookie(String date, String headerValue)
{
    if (!_cookieJar)
    {
        return;
...
bool HTTPClient_gsm::generateCookieString(String *cookieString)
{
    time_t now_local = time(NULL);
    time_t now_gmt = mktime(gmtime(&now_local));
